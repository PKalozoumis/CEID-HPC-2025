...
#N_tests = list(range(250, 1250, 250))

#Each row is for a different N value
#Each column is for a different type of time (e.g. gpu time)
times = np.zeros((len(N_tests), 4), dtype=np.float64)

#Run OpenMP program multiple times
...
try:
    for i, N in enumerate(N_tests):
        print(f"Running OpenMP program with N = {N}\n" + "="*64 + "\n")

        subprocess.run(["./gpu_omp", str(N), shm.name])
...
df = pd.DataFrame(times, columns=["Init", "CPU", "GPU", "Comparison"], index=N_tests)
