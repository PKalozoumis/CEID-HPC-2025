...
static inline float weno_minus_core_original(const float a, const float b, const float c, const float d, const float e)
{
	...
	const float* restrict const d, const float* restrict const e, float* restrict const out, const int NENTRIES)
{
	for (int i=0; i<NENTRIES; ++i)
		out[i] = weno_minus_core_original(a[i], b[i], c[i], d[i], e[i]);
}